...
public class RouteParseService(DbNavdataAdapter navdata)
{
    ...
    public async Task<List<FlightLeg>> ParseRouteAsync(string route, string dep, string arr)
    {
        var parser = new RouteParser($"{dep} {route} {arr}", Navdata);
        return await parser.Parse();
    }

    public async Task<IList<Violation>> ValidateFlight(Flight flight, List<FlightLeg> legs)
    {
