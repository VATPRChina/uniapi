...
public class AuthenticationEventHandler : JwtBearerEvents
{
    ...
    protected Task Validate(TokenValidatedContext context)
    {
        ...
        {
            throw new ApiError.InvalidToken("za_no_principal", "no principal in token", null);
        }
        var subject = context.Principal.FindFirstValue(ClaimTypes.NameIdentifier) ??
            throw new ApiError.InvalidToken("za_no_subject", "no subject in token", null);
        if (Ulid.TryParse(subject, out var _) == false)
        {
            throw new ApiError.InvalidToken("za_invalid_subject", "subject is not ulid", null);
        }
        var updatedAtStr = context.Principal.FindFirstValue(Services.TokenService.JwtClaimNames.UpdatedAt) ??
            throw new ApiError.InvalidToken("za_no_updated_at", "no updated_at in token", null);
        if (long.TryParse(updatedAtStr, out var _) == false)
        {
            throw new ApiError.InvalidToken("za_invalid_updated_at", "updated_at is not number", null);
        }
